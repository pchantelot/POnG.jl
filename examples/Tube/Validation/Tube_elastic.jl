...
# load Disperse solutions
data = load(joinpath(@__DIR__,"disperse_tube_axial.jld2"))
...
with_theme(theme_latexfonts()) do 
    fig = Figure()
        ...
        lines!(ax,data["k"] * h, data["f"] * h / sqrt(μs/ρs), color = :grey)
    display(fig)
end
# NB: The disperse solution does not show the torsion mode