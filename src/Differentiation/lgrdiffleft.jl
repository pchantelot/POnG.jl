...
Compute the first order differentiation matrix of size `n` × `n` associated to the Legendre Gauss Radau points vector `x` with x(1) = -1.
Refer to L. N. Trefethen: Spectral Methods in Matlab(2000). Based on lgrdiff.m (SPECTRAL METHODS: Algorithms, Analysis and Applications, Jie Shen et al.).
"""
